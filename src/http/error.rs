...
impl IntoResponse for Error {
    fn into_response(self) -> Response {
        match self {
            ...
            Self::Sqlx(ref e) => {
                // Using tracing's error macro with structured fields
                error!(
                    error.msg = %e,
                    error.kind = "sqlx",
                    error.detail = ?e,
            ...
            Self::Anyhow(ref e) => {
                error!(
                    ...
                    error.kind = "anyhow",
                    error.detail = ?e,
                    error.chain = ?e.chain().collect::<Vec<_>>(),
    ...
    fn on_constraint(
        ...
        self.map_err(|e| match e.into() {
            Error::Sqlx(sqlx::Error::Database(dbe)) if dbe.constraint() == Some(name) => {
                error!(constraint = name, "Database constraint violation");
                map_err(dbe)
            }
            e => e,
        })
