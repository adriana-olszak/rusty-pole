...
impl AuthUser {
    ...
    fn from_authorization(ctx: &ApiContext, auth_header: &HeaderValue) -> Result<Self, Error> {
        ...
        let jwt = jwt.verify_with_key(&hmac).map_err(|e| {
            debug!(error = %e, "JWT failed to verify");
            Error::Unauthorized
        })?;
    ...
    S: Send + Sync,
{
    ...
    async fn from_request(req: Request<Body>, state: &S) -> Result<Self, Self::Rejection> {
        let ctx = match req.extensions().get::<ApiContext>() {
            Some(ctx) => ctx,
            None => {
                debug!("ApiContext was not added as an extension");
                return Err(Error::Unauthorized);
            }
        };
...
#[async_trait]
impl<S> FromRequest<S> for MaybeAuthUser
where
    S: Send + Sync,
{
    type Rejection = Error;
